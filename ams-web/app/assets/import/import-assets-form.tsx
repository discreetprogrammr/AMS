...
export function ImportAssetsForm() {
  ...
  async function handleParse(e: React.FormEvent) {
    ...
    try {
      ...
      const p = await parseAssetsImport(formData);
      setPreview(p);
      setPhase("preview");
    } catch (err) {
  ...
  async function handleCommit() {
    ...
    setError(null);
    try {
      const r = await commitAssetsImport(preview.valid);
      setResult(r);
      setPhase("done");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Something went wrong.");
        ...
        <form onSubmit={handleParse} className="space-y-4 rounded-xl border border-hairline bg-surface p-6">
          <div>
            ...
            <p className="mt-1 text-[11px] text-slate-500">
              ...
              <a href="/api/assets/import/template" className="text-blue-400 hover:underline">
        ...
        <div className="space-y-4">
                ...
                <table className="w-full text-left text-xs">
                  <thead className="text-slate-500">
                    <tr>
                      ...
                      <th className="py-1 pr-3">Equipment Type</th>
                  ...
                  <tbody className="text-ink-soft">
                    {preview.valid.slice(0, 10).map((r) => (
                      <tr key={r.row} className="border-t border-hairline">
                        ...
                        <td className="py-1 pr-3">{r.organization_name}</td>
                        <td className="py-1 pr-3">{r.site_address ?? "—"}</td>
                        <td className="py-1 pr-3">{r.equipment_type}</td>
                        <td className="py-1 pr-3">{r.serial_number ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="rounded-lg bg-blue-600 px-5 py-2 text-sm font-medium text-ink hover:bg-blue-500 disabled:opacity-50"
            >
              {busy ? "Importing…" : `Import ${preview.valid.length} Asset${preview.valid.length === 1 ? "" : "s"}`}
            </button>
            <button
              type="button"
              onClick={reset}
      ...
      {phase === "done" && result && (
        <div className="space-y-4">
          <p className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-400">
            Imported {result.imported} asset{result.imported === 1 ? "" : "s"}.
          </p>
