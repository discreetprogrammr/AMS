...
## Follow-up — Report field refinements against real Astrophysics templates

After the Reports tab expansion above shipped its fields as reasonable guesses, the Super Admin provided Astrophysics' actual XIS Installation/Radiation Survey, Site Survey, and Training Record PDF forms — the manufacturer's real field paperwork — "as a guide only." Rather than either ignoring them or doing a rigid 1:1 replication (which would have meant a 29-step engineering installation checklist, per-attendee signature pads, and 15 new site-contact fields), each of the four new report types was refined with judgment calls about what's genuinely worth adding versus overkill for this app's scope:

- **Radiation Survey Test** — the real form's units are **µR/hr** against a **500 µR/hr US Federal limit**, not the mR/hr guess originally used, and the real named measurement points are **Generator Side Panel, LXDA Side Panel, Entry End, Exit End, Top View** — the form's defaults now match. Added a **Manufacturer** field alongside the existing meter Model/Serial/Cal Date, a separate **Background Radiation Reading** (the ambient reading taken before testing the unit itself), and a **Safety Devices & Warning Labels** checklist covering the real form's three same-shaped sections (Warning Label Verification, X-Ray ON Indicator, Safety Devices and Interlocks) as one editable list.
- **Site Survey** — replaced the single free-text notes box with a proper **5-section, 15-item tap-cycle checklist** (Receiving Area, Access Path, Power & Space, Environment, Connectivity — condensed from the real form's much longer 3-page question list), reusing the exact same checklist UX and the same `service_record_checklist_items` table the PM form already uses.
- **Installation** — added **Installation Type** (Conventional/Cargo), **Parts Required** (yes/no + list), and **Optional/Software Features** checklists (Exit Roller, External UPS, Screener Assist, TIP, etc., from the real form). Composed into `findings` at submit time rather than new schema columns, same technique the Corrective report already uses for Fault/Root Cause/Action.
- **Training** — added **Length of Training** and a **Competence Assessment** checklist (Test Attached, Quiz Attached, Actual Instructions Performed by Trainee, Verbal Acknowledgement). Also composed into `findings`, no new columns.
- **Explicitly not replicated**: the real Installation form's 29-step technical procedure (AC line measurements, TP2/TP15 voltage test points, etc. — too Astrophysics-engineering-specific for this app), per-attendee digital signatures on Training (kept as a free-text name list), and the real Site Survey's 5-contact-role block (kept as guidance text in the notes box rather than 15 new fields).

**`schema_step42.sql`** (new) — **run this in Supabase before deploying.** Adds `survey_meter_manufacturer` (text), `background_radiation_reading` (text), and `safety_checklist` (jsonb) to `service_records`. Also fixes a latent RLS gap found while wiring up the Site Survey checklist: `service_record_checklist_items`'s original SELECT policy inner-joined `service_records` to `assets`, which silently hid checklist items from clients on any site-only record (`asset_id` null — exactly what Site Survey now commonly is). The policy now matches via asset **or** site, mirroring the fix `schema_step41.sql` already applied to `service_records` itself.

Verified with `npx tsc --noEmit` — clean aside from the two pre-existing, expected `Cannot find module` errors (`web-push`, `papaparse`).

## Setup steps

1. Run `schema_step42.sql` in Supabase.
2. Push to GitHub and let Vercel redeploy.
3. File a **Radiation Survey Test Report** — confirm the default readings show Generator Side Panel/LXDA Side Panel/Entry End/Exit End/Top View in µR/hr against a 500 µR/hr limit, fill in a Manufacturer and Background Radiation Reading, check off a few Safety Devices & Warning Labels items, and confirm all of it appears correctly on both the PDF and the printable HTML view.
...
- `app/error-logs/` — new **Error Logs** page, Super Admin-only (same precedent as Audit Log: raw stack traces/context aren't an everyday ops surface, and every error already shows up on `/alerts` for regular staff too). Filterable (Unresolved/All), expandable stack trace + context per entry, Mark Resolved action, and a **Send Test Error** button that fires the real pipeline end-to-end (`error_logs` insert → `alerts` insert → `notifyStaff()` email/push) without needing to actually break anything in production to verify it works. Added to the sidebar nav, gated `staffOnly` + `superAdminOnly`.
...
4. Click **Send Test Error** — confirms the real pipeline end-to-end without breaking anything for real. Confirm: a row appears in **Error Logs** tagged `test:manual` with a stack trace and context; a matching "Error: test:manual" entry appears in **Alerts** with Critical severity; and (if `STAFF_NOTIFICATION_EMAIL`/push are configured) the email/push notification arrives.
...
Real ask: the Super Admin wants to hand-pick which sidebar modules any individual staff or client account sees, beyond the existing coarse role-based gating (staffOnly/superAdminOnly in `components/sidebar.tsx`) — and, per a follow-up request, unchecking a module should really take it away, not just hide its sidebar link. So this is now enforced as a real block, not a cosmetic one: `lib/supabase/middleware.ts` (which already ran on every request to refresh the Supabase session and redirect signed-out users to `/login`) now also redirects a signed-in user straight to the Dashboard, with a "you don't have access to that page" banner, the moment they land on any page belonging to a module a Super Admin hid from them — whether they got there by a sidebar link (already wouldn't be shown) or a direct URL they happened to know.

One deliberate boundary: this covers **pages** (anything matched via `lib/nav-items.ts`'s module hrefs), not the underlying `/api/*` routes those pages call — the middleware's matcher already excluded `/api/*` before this feature existed (see its own comment, for the cron jobs' sake) and still does. In practice this means, e.g., hiding "Reports" blocks the Reports page and every report's printable view, but not a raw fetch to the CSV-export or per-report-PDF API endpoint if someone already knew its exact URL. Actual data access is still governed by RLS/role regardless either way — this feature controls what a person can reach through the app's own navigation, not a second data-security layer.
...
- `components/sidebar.tsx` — nav filtering now checks `profile.hidden_modules` after the existing role checks (it can only take a module away, never grant one the role check already blocked).
- `lib/supabase/middleware.ts` — the actual enforcement. Resolves the current request path to a module (longest-matching href, same rule the sidebar uses to decide what's "active") and redirects to `/dashboard?access_denied=1` if that module is in the signed-in user's `hidden_modules`. Skips entirely for Super Admins.
- `lib/nav-items.ts`'s new `ALWAYS_ACCESSIBLE_HREFS = ["/dashboard", "/user-access"]` — both are hard-exempt from ever being blocked or hidden, in the sidebar filter and the middleware check alike. Dashboard is the middleware's own redirect target (hiding it would cause a redirect loop the instant anyone hit a blocked page); User Access is the only page that can undo a block at all. Neither is even offered as a checkbox on the User Access page — both are excluded from `lib/nav-items.ts`'s shared `NAV_MODULES` list entirely, and `components/sidebar.tsx` adds them to the nav directly instead.
- `lib/supabase/profile.ts` — `Profile` type and `getProfile()`'s select both widened to include `hidden_modules`.
- `app/dashboard/page.tsx` — reads a new `access_denied=1` query param and shows a small amber banner explaining why someone landed there instead of where they were headed.
...
Real ask: rearrange the Dashboard the way you'd rearrange home-screen widgets on a phone — drag to move, pick a size, and have it stick for next time. The Dashboard's cards were previously laid out in fixed Tailwind grid divs — same content, same data queries, just no longer fixed in place.

**`schema_step45.sql`** — **run this in Supabase before deploying** (same as before, no new migration needed for this round — `dashboard_layout` is untyped `jsonb`, so it holds the new shape below without any schema change). Personal display preference every user sets for *themselves*, so it follows the `full_name`/`avatar_url` self-service pattern (schema_step37/38.sql): just widens the `authenticated` column grant, no new RLS policy needed since the existing "update own display name" policy (`id = auth.uid()`) already covers any granted column on your own row.

**Dependency — `react-grid-layout`**, in `package.json` `dependencies`, **plus `@types/react-grid-layout` in `devDependencies`** — this version does *not* ship its own bundled types (an earlier build assumed it did and dropped the `@types` package, which broke the Vercel build with `Could not find a declaration file for module 'react-grid-layout'`; fixed by re-adding it). Like every other library in this repo, it can't be installed inside this sandbox to typecheck against (same restriction already noted for `web-push`/`papaparse`) — `npx tsc --noEmit` will show a third `Cannot find module 'react-grid-layout'` error until `npm install` actually runs, which happens automatically as part of Vercel's build.

**Redesigned this round, based on real usage feedback — fixed size presets instead of freeform drag-resize:**
- **Every KPI tile is now its own widget.** Total Assets, % Operational, Attention, Down, Unserviceable, and Compliance <30d were one combined card; each is now separately movable and resizable.
- **Resizing is now three fixed presets — Small / Medium / Large — not a drag handle.** Every widget at a given size has the exact same footprint as every other widget at that size, chosen via small S/M/L buttons that appear top-right of each card in Edit Layout mode. This replaces the old corner-drag resize, which had a real bug: a resize you dragged wasn't reliably what got saved. Because a size choice is now just one of three fixed values written directly to state (not a continuous pixel delta captured mid-drag), that class of bug isn't possible anymore — what you click is exactly what gets saved.
- **Cards automatically show more or less detail at each size.** Small shows just the headline number; Medium adds the supporting detail (status pills, stats, list rows); Large adds everything, including footer context and any list a card shows expands to more rows. This required turning the card components into genuine Client Components (`app/dashboard/widget-cards.tsx`, new file) — a Server Component's output can't react to a size picked later, client-side.
- **Edit Layout is now a small pencil icon**, not a text button — Reset and Done are icon-only too (circular-arrow and checkmark).

**What's in the app now:**
- `app/dashboard/widget-cards.tsx` (new, Client Component) — every card (`KpiCard`, `ActiveTicketsCard`, `EquipmentHealthCard`, `SlaPerformanceCard`, `SlaHistoryCard`, `RecentActivityCard`, `EquipmentAlertsCard`, `ServiceDueCard`, `ComplianceWarrantyCard`, `QuickActionCenterCard`), each taking a `size: "sm" | "md" | "lg"` prop that drives how much detail it shows.
- `app/dashboard/dashboard-grid.tsx` — the grid itself. Drag-to-move only (no corner-resize handle); a `SIZE_DIMENSIONS` map derives each widget's actual grid footprint from its chosen size, so a saved size can never drift out of sync with its footprint. Uses `cloneElement` to hand the client-picked `size` into each widget's pre-built element.
- `app/dashboard/actions.ts` — `saveDashboardLayout()` unchanged in behavior, just storing `{i, x, y, size}` per widget instead of `{i, x, y, w, h}`.
- `app/dashboard/page.tsx` — `widgets: DashboardWidget[]` now lists every KPI tile separately and imports its card components from `widget-cards.tsx` instead of defining them locally (~570 lines shorter as a result). Role-conditional cards (Equipment Health vs. Quick Actions, Recent Activity vs. Equipment Alerts) still only appear for the right role.
- Below `sm` breakpoint (~640px, phone width), the grid is skipped entirely in favor of a plain stacked column, always shown at full (`lg`) detail — not editable at that width.

Verified with `npx tsc --noEmit` — clean aside from the *three* expected `Cannot find module` errors (`web-push`, `papaparse`, `react-grid-layout`), no new type errors introduced by this round of changes.

## Setup steps

1. Run `schema_step45.sql` in Supabase, if you haven't already (no new migration needed for this round).
2. Push to GitHub and let Vercel redeploy.
3. Open the Dashboard — confirm the KPI row now shows as 6 separate small tiles.
4. Click the pencil icon (top right) to enter Edit Layout — confirm you can drag a card to a new position, and that S/M/L buttons appear top-right of each card.
5. Click M or L on a card — confirm it visibly resizes to the uniform preset and shows more detail (stats, extra list rows, footer text as applicable).
6. Click the checkmark to save — refresh the page and confirm both the position **and** the size you picked persisted (this is the bug that's being fixed — double-check it specifically).
7. Click the pencil again, then the reset icon — confirm it snaps back to the default arrangement and that persists after a refresh too.
...
**Follow-on incident — the browser tab itself froze ("Page Unresponsive"), unrelated to the above.** After the region and timeout fixes, Vercel's own logs showed `/dashboard` and `/login` both responding fast and clean (`200`s, no delay) — but the browser tab itself locked up solid client-side. That's a different class of bug: something in the drag/resize Dashboard grid (`app/dashboard/dashboard-grid.tsx`, `react-grid-layout`) freezing the render thread after hydration, most likely a resize-observer/layout feedback loop, not yet root-caused.

**Emergency mitigation, currently live:** `app/dashboard/page.tsx` no longer renders `<DashboardGrid>` — widgets render in a plain static `grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3` instead, same data, no drag/resize/edit. `dashboard-grid.tsx` and `widget-cards.tsx` are untouched and still present, just not wired in, so re-enabling later is a one-line change back to `<DashboardGrid widgets={widgets} savedLayout={savedLayout} />` — but only once the freeze is actually reproduced and fixed outside of production (a real browser dev tools session — Performance tab / React Profiler — is needed to catch this properly; static code review wasn't able to pin it down with confidence).

## Follow-up — Re-investigating the Dashboard grid freeze (still unresolved, staying on the static grid)

Picked back up with live debugging via a connected browser session (Claude in Chrome), instead of the static-code-review-only approach above. Two genuine bugs were found and fixed along the way; the underlying hang is still not fully resolved, so the app is staying on the static grid mitigation above rather than shipping something that still fails.

**Ruled out: Supabase auth-js lock deadlock.** A well-documented bug in older `@supabase/supabase-js` (an orphaned `navigator.locks`-based mutex that can hang every subsequent auth-dependent call forever, fixed upstream in `2.107.0+`) looked like a strong match for the symptoms. Upgraded `@supabase/supabase-js` to `^2.112.4` and `@supabase/ssr` to `^0.12.5`. Live-tested after deploy: the exact same 300-second hang recurred. Confirmed via a runtime version log that the upgrade genuinely was deployed (`supabase-js=2.112.4 ssr=0.12.5`), so this wasn't a stale-build false negative — the lock bug, while real, was not (the whole of) the cause.

**Instrumentation added (`lib/supabase/timed.ts`, temporary).** Wraps every Supabase call on `/dashboard` (middleware's `getUser()`, `getProfile()`, the layout-row select, `getSlaPolicyMap`, and all ~18 queries in the big `Promise.all`) so each logs a `START` and an `OK/ERROR <ms>` line to Vercel Runtime Logs — Vercel's own Logs UI has no way to see per-query timing otherwise (clicking an individual "External API" row just copies its path). This is what made the rest of this investigation possible: it proved every single query was completing in well under a second, every time, even on requests that went on to hang for the full 300 seconds. The bottleneck was never Supabase.

**Bug #1, confirmed and fixed: `react-grid-layout`'s `WidthProvider` HOC.** `WidthProvider` is a class component that calls `this.setState({ width })` on *every* ResizeObserver notification with no check for whether the width actually changed — and class components re-render unconditionally on `setState` regardless of whether the new value differs. If a render ever nudges the container's measured width by even a sub-pixel (a scrollbar toggling, layout settling near the viewport edge), that re-triggers the observer → re-renders → can nudge it again, a loop that spans React's async render cycle and so never trips the browser's own same-frame ResizeObserver guard. Reproduced live: with `DashboardGrid` active, a plain sidebar navigation to `/dashboard` left the browser tab genuinely unresponsive (script injection and `document_idle` both timed out) — the *client* freeze the original incident above was never able to pin down. Fixed in `dashboard-grid.tsx` by dropping `WidthProvider` entirely for a small hand-rolled `useContainerWidth()` hook: same ResizeObserver-based measurement, but using `useState`'s functional update with an actual equality check, so React 18 bails out of re-rendering when the width hasn't changed. `width` is now passed directly to the raw `GridLayout` component. This is a real fix, worth keeping regardless of the section below.

**Bug #2, confirmed but not (yet) a full fix: widget elements passed as a prop instead of as children.** With the `WidthProvider` fix live, the client freeze was gone — but a *server-side* hang remained: Vercel logs showed `/dashboard` reliably reaching the final `return` statement in the page (confirmed by a `[timing] dashboard.reachedReturn` sentinel log — meaning every query had already finished) and then never responding, until Vercel's own 300-second watchdog killed the function (`Vercel Runtime Timeout Error: Task timed out after 300 seconds`). The one prop that differed structurally between the (always-fast) static grid and (hanging) `DashboardGrid` was `widgets`: an array of objects each carrying a fully-built `content: ReactElement` field (real Supabase data as props), handed to `DashboardGrid` — a Client Component — as a single named prop. Restructured so `widgets` is now plain, guaranteed-serializable metadata only (id/size/position, no elements), and the actual widget elements are passed the conventional way, as `children`, matched back to their metadata by array position. **This did not resolve the hang** — retested live afterward and got the identical `reachedReturn`-then-nothing-then-504 pattern, 100% reproducible, while the static grid stayed 100% reliable across every test today using the exact same underlying data.

**Where this leaves it:** the server-side hang is real, reproducible, and still unexplained. It happens specifically when `DashboardGrid` (not the static grid) is what receives the widget elements, after all data has loaded, somewhere inside Next.js's own server-side rendering/serialization of that component tree — a place static code review and redeploy-and-inspect-Vercel-logs cycles can't see into any further. Diagnosing it properly needs local reproduction: `next dev` with a real debugger or profiler attached (Node's `--inspect`, or the React DevTools Profiler), which wasn't available in this remote-deployment-only debugging session.

**Current state:** `DEBUG_STATIC_GRID` in `app/dashboard/page.tsx` controls this — currently `true` (static grid, safe, matches the emergency mitigation above). `DashboardGrid` itself carries both real fixes from this round (no `WidthProvider`, elements passed as children) and is otherwise unchanged; flipping the flag to `false` re-enables it for whenever the remaining hang gets properly root-caused with local tooling. `lib/supabase/timed.ts` and its call sites are still in place — genuinely useful if this needs revisiting, but worth stripping out eventually since they log on every request, not just `/dashboard`.

Verified with `npx tsc --noEmit` at each step — same three pre-existing `Cannot find module` errors (`web-push`, `papaparse`, `react-grid-layout`) throughout, no new ones introduced.
